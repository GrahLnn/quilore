use crate::domain::platform::twitter::auth::auth::{AuthGenerator, GuestAuth, UserAuth, auth_generator_for};
...
use crate::domain::models::userkv::{UserKey, get_userkv_value};
use anyhow::Result;
use std::sync::Once;
...
async fn test_guest_auth_generate() -> Result<()> {
    ...
    assert!(result.is_ok(), "GuestAuth generate failed: {:?}", result.err());
    ...
    assert!(headers.contains_key("x-guest-token"), "Guest token header not found");
...
async fn test_user_auth_generate() -> Result<()> {
    ...
    match result {
        Ok(credential) => {
            ...
            assert!(headers.contains_key("x-csrf-token"), "CSRF token header not found");
        },
        Err(e) => {
            // If there's an error, it should be because no Twitter cookie was found
            println!("UserAuth generate failed as expected: {}", e);
            assert!(e.to_string().contains("No Twitter cookie found") || 
                   e.to_string().contains("Failed to read stored Twitter cookie"),
                   "Unexpected error: {}", e);
        }
    }
